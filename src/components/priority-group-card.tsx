    ...
    <>
      <div className="relative">
        <div
          ref={ref}
          className={twMerge(
            'flex flex-col bg-slate-50 shadow rounded-lg gap-2 p-1.5',
            draggableState.type === 'is-dragging' && 'bg-slate-200 opacity-40',
            droppableState.isOver && 'bg-indigo-100'
          )}
        >
          <div className="flex items-center gap-2">
            <DragHandle
              ref={dragHandleRef}
              className="hover:bg-indigo-100 hover:text-indigo-600 rounded px-0.5 py-1"
            />
            <h2 className="text-md text-indigo-600 font-medium">
              {group.name}
            </h2>
          </div>

          {group.documents.length > 0 && (
            <div ref={containerRef} className="grid gap-2 pl-2">
        ...
        {droppableState.instruction && (
          <DropIndicator
            instruction={droppableState.instruction}
            lineGap="10px"
          />
        )}
      </div>
      {draggableState.type === 'preview' &&
        createPortal(
          <DragPreview
            className="bg-indigo-500 text-white text-sm"
            value={`${group.name}`}
          />,
